...
with open("test.wav","rb") as f:
    ...
    for i in range(0,byte_count,block_size):
        wav_data+=[int.from_bytes(f.read(block_size),"little")]
   
#Find minimum and maximum sample size
print("Min sample value:",min(wav_data))
print("Max sample value:",max(wav_data))
...
with open("wave.txt","wt") as f:
    for sample in wav_data:
        ...
        if sample<wave_min:
            wave_min=sample
    f.write("Wave min: "+str(wave_min)+"\n")
    f.write("Wave max: "+str(wave_max)+"\n")
